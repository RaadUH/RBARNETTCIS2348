...
def combination(): # returns the all the combinations of manufacturer and item type given in FullInventory
    test = ReadWrite()
...
def show_item(query): #Computes the resulted item with respect to query
    test = ReadWrite()
    dict1 = test.getCSVFileData("FullInventory")
    output = []
    for k,v in dict1.items():
...
def consider_item_result(query,price,itemID): #computes the item to considered other that the resulted item 
    test = ReadWrite()
    dict1 = test.getCSVFileData("FullInventory")
    output = []
    for k,v in dict1.items():
        date = datetime.strptime(v[3],'%m/%d/%Y')
        today_date = date.today()
        if(k!=itemID and v[1].lower().strip()==query[1] and date>=today_date and v[4]!='damaged' and v[2]==price):
            row = []
            row.append(k.strip())
            row.append(v[0].strip())
            row.append(v[1].strip())
...
while(inp!='q'):
    ...
    if(len(query)<2 or query[0]==query[1] or query not in combination()):
        ...
        if(len(result_item)==0):
            ...
            consider_item = consider_item_result(query,result_item[3],result_item[0])
